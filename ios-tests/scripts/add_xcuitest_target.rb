...
ui_target = project.targets.find { |t| t.name == UITEST_TARGET_NAME }
created = ui_target.nil?
...
if created
  ...
  ui_target = project.new_target(:ui_test_bundle, UITEST_TARGET_NAME, :ios, deployment_target)
...
group =
  project.main_group.children.find { |c| c.isa == "PBXGroup" && c.display_name == UITEST_TARGET_NAME } ||
  project.main_group.new_group(UITEST_TARGET_NAME, group_path)

swift_files = Dir.glob(SOURCES_DIR.join("**/*.swift")).sort
raise "No Swift sources found under #{SOURCES_DIR}" if swift_files.empty?

referenced = ui_target.source_build_phase.files.filter_map { |bf| bf.file_ref&.real_path&.to_s }
...
if created
  ...
  scheme = File.exist?(scheme_path) ? Xcodeproj::XCScheme.new(scheme_path.to_s) : Xcodeproj::XCScheme.new
...
if added.empty?
  puts "Sources are in sync — no files added."
else
  puts "Added #{added.length} source file(s): #{added.join(", ")}"
end
